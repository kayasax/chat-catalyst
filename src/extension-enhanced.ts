	...
	function buildRichContextPrompt(autoPrompt: string): string {
		...
		if (autoPrompt.length > 2000) {
			richPrompt = richPrompt.replace(autoPrompt,
				autoPrompt + '\n\n📎 Note: This is a detailed prompt. All relevant project files are attached above for full context.'
			);
		}
...
Full details are in the attached context file.`;
	...
	async function clearChatInput(): Promise<boolean> {
		...
		try {
			...
			try {
				...
				// console.log('✅ Text cleared via selectAll + deleteRight');
				return true;			} catch {
			...
			try {
				...
				await new Promise(resolve => setTimeout(resolve, 50));
				// console.log('✅ Text cleared via chat cancel');
				return true;			} catch {
	...
	async function injectAutoPrompt(): Promise<boolean> {
		...
		try {
			...
			console.log('🔄 Injecting enhanced prompt with attachments...');
			...
			try {
				...
				console.log('✅ Enhanced prompt with attachments injected successfully');
				vscode.window.setStatusBarMessage('✅ Rich prompt injected!', 2000);
				...
				try {
					...
					console.log('✅ Enhanced prompt injected via paste');
					vscode.window.setStatusBarMessage('✅ Rich prompt injected!', 2000);
	...
	const handler: vscode.ChatRequestHandler = async (request: vscode.ChatRequest, context: vscode.ChatContext, stream: vscode.ChatResponseStream, _token: vscode.CancellationToken) => {
		...
		try {
			...
			if (success) {
				stream.markdown('✅ Auto-prompt injected into main chat! You can now continue your conversation there.');
			} else {
				stream.markdown('⚠️ Could not inject into main chat. Here\'s your prompt:\n\n' + autoPrompt);
			}
		} catch (error) {
	...
	const startChatCommand = vscode.commands.registerCommand('chatCatalyst.startChat', async () => {
		...
		try {
			...
			console.log('🚀 Starting chat with enhanced prompt injection...');
			...
			try {
				...
				try {
					...
					console.log('✅ Chat focused, injecting enhanced prompt...');
					const success = await injectAutoPrompt();

					if (success) {
						console.log('🎉 Enhanced prompt injection complete!');
						vscode.window.setStatusBarMessage('🎉 Ready to chat with full context!', 3000);
					} else {
	...
	const toggleCommand = vscode.commands.registerCommand('chatCatalyst.toggle', async () => {
		...
		vscode.window.showInformationMessage(`✅ Chat Catalyst ${status}!`);
	});
...
- Active editor: ${vscode.window.activeTextEditor ? vscode.workspace.asRelativePath(vscode.window.activeTextEditor.document.uri) : 'None'}
	...
	if (!hasPrompt) {
		vscode.window.showInformationMessage(
			'🚀 Chat Catalyst is ready! Set up your auto-prompt to get started.',
			'Set Up Prompt',
			'Learn More'
		).then(selection => {
...
export function deactivate() {
	console.log('👋 Chat Catalyst extension deactivated');
}
