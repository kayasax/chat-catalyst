...
export function activate(context: vscode.ExtensionContext) {
	...
	console.log('🧪 TESTING VERSION: catalyst-testing.vsix - THIS IS NOT THE MARKETPLACE VERSION');
	console.log('📦 Extension Location:', context.extensionPath);
	...
	function cleanup() {
		...
		state.disposables = [];
		console.log('✅ Disposed all event listeners');
	...
	async function createPromptContextFile(longPrompt: string): Promise<string | null> {
		try {
			...
			const formattedContent = `# 🧠 Chat Context for Session
...
`;			fs.writeFileSync(contextFile, formattedContent);
			...
			console.log(`✅ Temp context file created: ${contextFile}`);
			console.log(`📌 Temp file will be cleaned up only on extension deactivation`);
	...
	function getRelevantProjectFiles(): string[] {
		...
		for (const { path: sessionPath, ref } of sessionStarterPaths) {
			console.log(`🔍 Checking for Session_starter.md at: ${sessionPath}`);
			if (fs.existsSync(sessionPath)) {
				const stats = fs.statSync(sessionPath);
				console.log(`📄 Found Session_starter.md (${stats.size} bytes): ${ref}`);
				if (stats.size > 0) { // Only add if file has content
					relevantFiles.push(ref);
					console.log(`✅ Added Session_starter.md to relevant files: ${ref}`);
					break;
				} else {
					console.log(`⚠️ Session_starter.md exists but is empty, skipping: ${ref}`);
				}
			}
		}
	...
	async function buildSmartContextPrompt(autoPrompt: string): Promise<string> {
		...
		console.log(`� Found relevant files: ${relevantFiles.join(', ')}`);
		...
		if (sessionStarter) {
			console.log(`📎 Using existing Session_starter.md instead of creating context file`);
...
Ready to help with your project! 🚀`;
			...
			console.log(`🎯 Session starter prompt generated (${richPrompt.length} chars)`);
		...
		if (autoPrompt.length > 1000) {
			console.log(`🔄 Long prompt detected (${autoPrompt.length} chars) and no Session_starter.md found, creating context file...`);
			const contextFile = await createPromptContextFile(autoPrompt);
			if (contextFile) {
				console.log(`✅ Context file created at: ${contextFile}`);
				try {
					...
					if (!fs.existsSync(vscodeDir)) {
						console.log(`📁 Creating .vscode directory: ${vscodeDir}`);
						fs.mkdirSync(vscodeDir, { recursive: true });
					}

					console.log(`📋 Copying context file from: ${contextFile}`);
					console.log(`📋 Copying context file to: ${workspaceContextFile}`);

					try {
						fs.copyFileSync(contextFile, workspaceContextFile);
						console.log(`✅ Context file copied successfully`);
					} catch (copyError) {
						console.error(`❌ Failed to copy context file:`, copyError);
						throw copyError;
					}

					// Verify the file was actually created
					if (!fs.existsSync(workspaceContextFile)) {
						console.error(`❌ Context file was not created at: ${workspaceContextFile}`);
					...
					console.log(`✅ Context file created: .vscode/${contextFileName}`);
					console.log(`📌 Context file will persist for 24 hours to support #file: reference`);
					...
					const cleanupTimer = createManagedTimer(() => {
						try {
							if (fs.existsSync(workspaceContextFile)) {
								...
								console.log(`🧹 Cleaned up context file after 24 hours: .vscode/${contextFileName}`);
					...
					console.log(`⏰ Cleanup timer set for 24 hours, timer ID: ${cleanupTimer}`);
					console.log(`📁 File will remain accessible at: .vscode/${contextFileName}`);					richPrompt += `

🎯 **Long Prompt Context Loaded**
...
Ready to help with your project! 🚀`;
					...
					console.log(`🎯 Final rich prompt generated (${richPrompt.length} chars)`);
					console.log(`📋 Prompt preview: ${richPrompt.substring(0, 200)}...`);
...
⚠️ **Context File Attachment Failed**
...
Ready to help with your project! 🚀`;
	...
	async function injectSmartPrompt(): Promise<boolean> {
		...
		console.log(`🔄 Processing prompt (${autoPrompt.length} characters)...`);
		...
		try {
			...
			try {
				...
				console.log('✅ Smart prompt injected successfully');
				vscode.window.setStatusBarMessage(
					smartPrompt.length > autoPrompt.length ?
					'✅ Enhanced prompt with attachments injected!' :
					'✅ Prompt injected!',
					3000
				...
				try {
					...
					console.log('✅ Smart prompt pasted successfully');
					vscode.window.setStatusBarMessage('✅ Smart prompt pasted!', 2000);
					return true;
				} catch {
					console.log('❌ Both type and paste failed');
					return false;
				}
			}
		} catch (error) {
			console.error('❌ Smart injection failed:', error);
	...
	const editPromptCommand = vscode.commands.registerCommand('chatCatalyst.editPrompt', async () => {
		...
		if (newPrompt !== undefined) {
			...
			vscode.window.showInformationMessage('✅ Auto-prompt updated successfully!');
		}
	});
	...
	async function createCustomInstructions(): Promise<void> {
		...
		// Ensure .github and .github/prompts directories exist
	...
	// Create session startup prompt file in .github/prompts/
	async function createSessionStartupPrompt(): Promise<void> {
		const workspaceFolder = vscode.workspace.workspaceFolders?.[0];
		if (!workspaceFolder) {
			return;
		}

		const promptsDir = path.join(workspaceFolder.uri.fsPath, '.github', 'prompts');
		const sessionStartupPromptPath = path.join(promptsDir, 'session-startup.prompt.md');

		// Don't overwrite existing file
		if (fs.existsSync(sessionStartupPromptPath)) {
			return;
		}
		...
		const sessionStartupContent = `# Session Startup Prompt
...
`;
		...
		fs.writeFileSync(sessionStartupPromptPath, sessionStartupContent);
		console.log(`✅ Created session startup prompt: ${sessionStartupPromptPath}`);
	}
...
Ready to continue where we left off! 🚀`;
	...
	async function enhancedStartChat(): Promise<void> {
		...
		try {
			...
			console.log('🧪 USING TESTING VERSION: catalyst-testing.vsix');
			...
			// Always create/update session startup prompt file
			await createSessionStartupPrompt();
			const sessionStartupPromptPath = path.join(workspaceFolder.uri.fsPath, '.github', 'prompts', 'session-startup.prompt.md');
			if (!fs.existsSync(sessionStartupPromptPath)) {
				// This should not happen since we just created it, but let's be safe
				setupChanges.push('Created .github/prompts/session-startup.prompt.md for automated session continuity');
			}
